"""
Deep-Risk-OPP — 数据管道共享层 (P1-2 合并 gor_daily/weekly_data_pull)
数据源: FRED + akshare + yfinance + Web备用
"""
...
def generate_alerts(gor_output):
    ...
    alloc = gor_output['allocation']
    wti = data.get('WTI原油', {}).get('price')
    vix = data.get('VIX恐慌指数', {}).get('price')
    dxy = data.get('美元指数DXY', {}).get('price')
    us10y = data.get('10Y美债收益率', {}).get('price')

    alerts = []
    today = datetime.date.today().strftime('%Y-%m-%d')

    if wti and wti < 75:
        alerts.append(("🔴 CRITICAL", f"WTI ${wti:.2f} < $75 — Hard stop active. Oil forced ≤ 5%."))
    if wti and wti < 70:
        alerts.append(("🔴🔴 BREAKER", f"WTI ${wti:.2f} < $70 — Circuit breaker escalation. Check liquidity."))
    if vix and vix > 25:
        alerts.append(("🔴 CRITICAL", f"VIX {vix:.2f} > 25 — Vol explosion. Risk positions -50%."))
    if vix and vix > 20:
        alerts.append(("🟠 WARNING", f"VIX {vix:.2f} > 20 — Elevated volatility. Tighten stops."))
    if us10y and us10y > 4.5:
        alerts.append(("🟠 WARNING", f"10Y {us10y:.2f}% > 4.5% — Rate pressure escalating."))
    if dxy and dxy > 101:
        alerts.append(("🟠 WARNING", f"DXY {dxy:.2f} > 101 — Centripetal collapse accelerating."))
    if vix and vix < 17:
        alerts.append(("🟢 OPPORTUNITY", f"VIX {vix:.2f} < 17 — SPY Straddle window. Cost = 2-3% of portfolio."))

    if alerts:
        alert_path = BASE_DIR / "看板日志" / f"ALERT_{today}.md"
        lines = [f"# 🚨 Deep-Risk-OPP Alert · {today}", ""]
        lines.append(f"GOR(WTI)={gor_output['gor_wti']} | GOR(Brent)={gor_output['gor_brent']}")
        lines.append(f"Oil={alloc['油气']}% | Gold={alloc['黄金']}% | Cash={alloc['现金']}%")
        lines.append("")
        for level, msg in alerts:
            lines.append(f"- {level}: {msg}")
        lines.append("")
        lines.append("---")
        lines.append("*Auto-generated by gor_daily.py. Research framework, not investment advice.*")
        with open(alert_path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(lines))
        log(f"  ALERT: {len(alerts)} triggers written to {alert_path.name}")
    return alerts
