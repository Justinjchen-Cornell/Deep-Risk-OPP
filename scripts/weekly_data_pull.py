...
def log(msg):
    ...
    try:
        ...
        # Fallback: strip non-ASCII for console, full text goes to file
...
def fetch_all():
    ...
    try:
        ...
        if data.get('wti'):
            ...
            log(f"  Brent: ${data['brent']}/bbl (from WTI*1.06, API failed)")
        else:
    ...
    # 5. 10Y美债 — 取最后一笔有美国10Y数据的行
    try:
        df = ak.bond_zh_us_rate()
        # Find last row with valid US 10Y data
        col_name = None
        for c in df.columns:
            if '美国国债收益率10年' in c or ('10' in c and '美国' in c):
                ...
                break
        if col_name is None:
            col_name = '美国国债收益率10年'  # default, may fail
        valid = df[df[col_name].notna()]
        if len(valid) > 0:
            last = valid.tail(1).iloc[0]
            data['us10y'] = round(float(last[col_name]), 2)
            log(f"  10Y: {data['us10y']}%")
        else:
            raise ValueError("No valid US 10Y data")
    except Exception as e:
        log(f"  10Y ERROR: {e}"); data['us10y'] = None

    # 6. DXY (Trading Economics scrape)
    try:
    ...
    # 7. VIX (CBOE scrape, with TradingView fallback)
    try:
        # Try TradingView API first (more reliable)
        ...
        if tv_data.get('data') and len(tv_data['data']) > 0:
            ...
            raise ValueError("Empty TV data")
    except:
        # Fallback: CBOE page
        try:
            ...
            for pat in [r'VIX Spot Price.*?(\d{1,2}\.\d{2})', r'as of.*?(\d{1,2}\.\d{2})', r'"price":\s*(\d{1,2}\.\d{2})']:
                m = re.search(pat, html, re.DOTALL)
                if m:
                    data['vix'] = float(m.group(1))
                    break
            if data.get('vix'):
                log(f"  VIX: {data['vix']} (CBOE)")
            else:
                raise ValueError("VIX not found in CBOE page")
...
def save_gor_json(data, gor):
    ...
    output = {
        ...
        "capital_three_flows": {
            "total": "收缩",
            "direction": "向心坍缩" if (data.get('dxy') and data['dxy'] > 99) else "中性",
            "speed": "加速中" if (data.get('vix') and data['vix'] > 18) else "放缓",
            "dxy": data.get('dxy'), "vix": data.get('vix'),
            "tenyear": data.get('us10y'), "warsh_alert": True
        },
