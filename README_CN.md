...
>
> **诚实审计更新（2026-09）**：我们对「45」主张做了 56 年全样本压力审计。方向成立，但证据比听上去薄——独立宏观情境只有 **2–3 个**（"历史"主要是 2020 和当下），且最强月份彼此重叠（照片数 ≠ 事件数）。结论：GOR≥45 是**极端标记，不是概率规律**；**恐慌是必要条件**（历史统计：恐慌期 82.6% 均值回归 vs 平静期 47.8%）。现在用该信号配油气前，先过**机制筛选器**：拆腿 · 恐慌签名 · 油迫近 → 🟢加仓 / 🟡观察 / 🔴该逻辑下不新建仓。详见 [`frameworks/01-GOR方向框架.md`](frameworks/01-GOR方向框架.md) ｜ 完整审计：[Quant-Modeling-Dojo](https://github.com/Justinjchen-Cornell/Quant-Modeling-Dojo)
